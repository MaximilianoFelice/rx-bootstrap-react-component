...
var InputField = exports.InputField = function (_BaseComponent) {
  ...
  function InputField(props) {
    ...
    }).merge(new _rx2.default.BehaviorSubject(_this.state.labelProps)).filter(_helpers.isDefined);

    _this.inputObs = _this.props.observeOn.map(function (state) {
      return state && state.inputProps;
    }).merge(new _rx2.default.BehaviorSubject(_this.state.inputProps)).filter(_helpers.isDefined);

    _this.errorsObs = _this.props.observeOn.map(function (state) {
      return state.errors;
    }).merge(new _rx2.default.BehaviorSubject(_this.state.errors)).filter(_helpers.isDefined).map(function (errors) {
  ...
  _createClass(InputField, [{
    ...
    value: function render() {
      return _react2.default.createElement(
        ...
        _react2.default.createElement(InputErrors, { observeOn: this.errorsObs })
      );
    }
  }]);
